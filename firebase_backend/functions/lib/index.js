...
exports.cleanupStaleDevices = exports.dispatchPublicPublishedPostDigest = exports.sendMeetingReminders = exports.executeScheduledCampaigns = exports.sendMessageCampaign = exports.sendTestFcmToSelf = exports.onMeetingRsvpWritten = exports.onMeetingInterestWritten = exports.onMeetingInterestCreated = exports.onMeetingCreated = exports.onBreakingNewsCreated = exports.onReporterApplicationDecision = exports.onUserProfileIdentityChanged = exports.onUserRoleChanged = exports.onCommentCreated = exports.onPostResubmitted = exports.onPostPublishedOutboxEnqueue = exports.onPostStatusChange = exports.onPostCreated = exports.servePostPreview = exports.getTrendingHashtags = exports.trackShareInteraction = exports.deletePost = exports.createUserWithRole = exports.updateUserRole = exports.createComment = exports.togglePostInteraction = exports.recomputeAllMeetingEngagementCounts = exports.getCurrentStorageUsage = exports.updateStorageConfig = exports.moderatePost = exports.backfillAdminPostAuthorNames = exports.backfillPostTranslations = exports.translateText = exports.createPost = exports.backfillUserPhoneNormalized = exports.verifyMsg91OtpAndExchangeToken = void 0;
...
(0, v2_1.setGlobalOptions)({ region: 'asia-south1', maxInstances: 3 });
...
function parseRuntimeInt(value, fallback, { min, max, }) {
...
const otpVerifyMinInstances = parseRuntimeInt(process.env.OTP_VERIFY_MIN_INSTANCES, 0, { min: 0, max: 2 });
const otpVerifyMaxInstances = parseRuntimeInt(process.env.OTP_VERIFY_MAX_INSTANCES, 3, { min: 1, max: 30 });
function ensureAuth(uid) {
    if (!uid) {
...
function ensureRole(callerRole, minimumRole) {
    if (roleWeight(callerRole) < roleWeight(minimumRole)) {
        throw new https_1.HttpsError('permission-denied', 'Insufficient role permission.');
    }
}
function canonicalPostAuthorName(role, displayNameRaw) {
...
async function writeAuditEvent({ eventType, entityType, entityId, actorId, actorRole, summary, metadata, before, after, }) {
    const normalizedEventType = String(eventType).trim();
    const normalizedEntityType = String(entityType).trim();
    if (!normalizedEventType || !normalizedEntityType)
        return;
    const doc = {
...
function roleFromConfiguredPhone(phone) {
    if (SUPER_ADMIN_PHONES.has(phone))
        return 'super_admin';
    if (ADMIN_PHONES.has(phone))
        return 'admin';
    if (REPORTER_PHONES.has(phone))
        return 'reporter';
    return null;
}
...
function buildDisplayName(normalizedPhone, data) {
...
function serializeUser(uid, data) {
    return {
        id: uid,
        phone_number: String(data?.phone_number ?? '').trim() || data?.phone_normalized || '',
...
async function ensureFirebaseUser(uid, displayName) {
...
function isMsg91Success(data) {
    ...
    return (String(payload.type ?? '').toLowerCase() == 'success' ||
...
async function verifyMsg91AccessToken(accessToken, expectedPhone) {
    ...
    if (normalizedToken.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'Access token is empty.');
    }
    ...
    try {
        ...
        if (typeof payload.exp === 'number' && payload.exp < now) {
            throw new https_1.HttpsError('unauthenticated', 'Access token has expired.');
        }
        // Check issued-at is not in the far future (clock-skew tolerance: 5 min)
        if (typeof payload.iat === 'number' && payload.iat > now + 300) {
            throw new https_1.HttpsError('unauthenticated', 'Access token has invalid timestamps.');
        }
        // Verify phone number if provided
        if (expectedPhone && expectedPhone.length >= 10) {
            const tokenPhone = normalizeIndianPhone(payload.identifier) ||
            ...
            if (tokenPhone.length > 0 && tokenPhone !== expectedPhone) {
                throw new https_1.HttpsError('unauthenticated', 'Access token phone does not match.');
            }
        }
        console.log('[verifyMsg91AccessToken] JWT verification succeeded (fast path)', JSON.stringify({
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        console.error('[verifyMsg91AccessToken] JWT decode/validation failed:', error.message);
        throw new https_1.HttpsError('unauthenticated', 'OTP verification failed.');
    }
}
async function getCallerProfile(uid) {
    const doc = await db.collection('users').doc(uid).get();
    if (!doc.exists || !doc.data()) {
        throw new https_1.HttpsError('permission-denied', 'User profile is missing or not provisioned.');
    }
    const data = doc.data();
    if (data.is_active == false) {
        throw new https_1.HttpsError('permission-denied', 'User account is disabled.');
    }
    return { data, role: normalizeRole(data.role) };
}
exports.verifyMsg91OtpAndExchangeToken = (0, https_1.onCall)({
    invoker: "public",
    ...
    const normalizedPhone = normalizeIndianPhone(input.phone_number ?? input.phoneNumber);
    ...
    if (!isValidIndianPhone(normalizedPhone) || accessToken.length == 0) {
        throw new https_1.HttpsError('invalid-argument', 'A valid phone number and Msg91 access token are required.');
    }
    ...
    try {
        ...
        const ce = claimErr;
        if (ce.code === 'auth/user-not-found' ||
            ce.code === 'auth/configuration-not-found') {
            console.warn(`[setCustomUserClaims] skipped (${ce.code}) — claims in custom token`);
        }
        else {
            throw claimErr;
        }
...
exports.backfillUserPhoneNormalized = (0, https_1.onCall)(async (request) => {
...
exports.createPost = (0, https_1.onCall)(async (request) => {
    ...
    if (caption.length == 0 || contentType.length == 0 || category.length == 0) {
        throw new https_1.HttpsError('invalid-argument', 'caption, contentType, and category are required.');
    }
    ...
    const poemVerses = Array.isArray(input.poemVerses)
        ...
        : [];
    const sourceLanguage = (0, translation_service_1.detectLanguageCode)([caption, articleContent, poemVerses.join('\n')].join('\n'));
    const sourceHash = (0, translation_service_1.computePostSourceHash)({
        caption,
...
exports.translateText = (0, https_1.onCall)(async (request) => {
    throw new https_1.HttpsError('failed-precondition', 'Cloud translation is disabled. Use on-device ML Kit translation.');
});
exports.backfillPostTranslations = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ensureRole(caller.role, 'admin');
    return {
...
exports.backfillAdminPostAuthorNames = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ensureRole(caller.role, 'admin');
    let scanned = 0;
    ...
    while (true) {
        ...
        if (userIdsToLoad.size > 0) {
            for (const chunk of Array.from(userIdsToLoad).reduce((acc, id, idx) => {
        ...
        for (const doc of snap.docs) {
            ...
            const isAdminAuthored = resolvedRole === 'admin' || resolvedRole === 'super_admin';
...
exports.moderatePost = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ensureRole(caller.role, 'admin');
    const input = toMap(request.data);
    ...
    if (postId.length == 0 ||
        ...
        throw new https_1.HttpsError('invalid-argument', 'postId and a valid moderation status are required.');
    }
    if (status === 'rejected' && rejectionReason.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'rejectionReason is required when rejecting a post.');
    }
...
exports.updateStorageConfig = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ...
    const parseLimit = (raw, key) => {
        ...
        if (!Number.isFinite(value) || value <= 0) {
            throw new https_1.HttpsError('invalid-argument', `${key} must be a positive number.`);
        }
        if (value > 5000) {
            throw new https_1.HttpsError('invalid-argument', `${key} is too large.`);
        }
        return Number(value.toFixed(2));
    };
    const parseUtilised = (raw, key) => {
        const value = Number(raw);
        if (!Number.isFinite(value) || value < 0) {
            throw new https_1.HttpsError('invalid-argument', `${key} must be a non-negative number.`);
        }
        if (value > 5000) {
            throw new https_1.HttpsError('invalid-argument', `${key} is too large.`);
        }
    ...
    const interactionsLimit = parseLimit(input.interactions_limit_gb, 'interactions_limit_gb');
    ...
    const postsUtilised = parseUtilised(input.posts_utilised_gb ?? input.posts_used_gb ?? 0, 'posts_utilised_gb');
    const interactionsUtilised = parseUtilised(input.interactions_utilised_gb ?? input.interactions_used_gb ?? 0, 'interactions_utilised_gb');
    const usersUtilised = parseUtilised(input.users_utilised_gb ?? input.users_used_gb ?? 0, 'users_utilised_gb');
    const systemUtilised = parseUtilised(input.system_files_utilised_gb ?? input.system_used_gb ?? 0, 'system_files_utilised_gb');
...
exports.getCurrentStorageUsage = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ensureRole(caller.role, 'admin');
    ...
    const sumFileBytes = async (prefix) => {
        try {
    ...
    const [postsCountSnap, usersCountSnap, commentsCountSnap, interactionsCountSnap, postsBytes, totalBucketBytes] = await Promise.all([
    ...
    const estimatedInteractionsBytes = (commentsCount * 850) + (interactionsCount * 250);
...
exports.recomputeAllMeetingEngagementCounts = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    ensureRole(caller.role, 'admin');
    let scanned = 0;
    ...
    while (true) {
        ...
        if (snap.empty)
            break;
        const results = await Promise.all(snap.docs.map(async (meetingDoc) => {
            scanned += 1;
            try {
...
exports.togglePostInteraction = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const input = toMap(request.data);
    const postId = String(input.postId ?? '').trim();
    const type = String(input.type ?? '').trim();
    if (postId.length == 0 || !['like', 'bookmark'].includes(type)) {
        throw new https_1.HttpsError('invalid-argument', 'postId and a valid interaction type are required.');
    }
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (type == 'bookmark') {
            if (next) {
                tx.set(bookmarkRef, { created_at: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
            }
            else {
...
exports.createComment = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const caller = await getCallerProfile(uid);
    const input = toMap(request.data);
    const postId = String(input.postId ?? '').trim();
    ...
    if (postId.length == 0 || content.length == 0) {
        throw new https_1.HttpsError('invalid-argument', 'postId and content are required.');
    }
    const ref = await db
        .collection('posts')
        .doc(postId)
...
exports.updateUserRole = (0, https_1.onCall)(async (request) => {
    ...
    if (roleWeight(newRole) >= roleWeight(caller.role) && caller.role !== 'super_admin') {
        throw new https_1.HttpsError('permission-denied', 'Cannot assign a role equal to or above your own.');
    }
    ...
    if (caller.role === 'admin' && roleWeight(newRole) >= roleWeight('admin')) {
        throw new https_1.HttpsError('permission-denied', 'Admins can only assign reporter or public_user roles.');
    }
    ...
    if (targetDoc.exists) {
        ...
        if (targetRole === 'super_admin' && caller.role !== 'super_admin') {
            throw new https_1.HttpsError('permission-denied', 'Cannot modify a super admin.');
        }
    }
    // Update Firestore
    ...
    try {
        ...
        if (ce.code !== 'auth/user-not-found' && ce.code !== 'auth/configuration-not-found') {
            console.warn('[updateUserRole] setCustomUserClaims failed:', claimErr.message);
        }
    }
    await writeAuditEvent({
...
exports.createUserWithRole = (0, https_1.onCall)(async (request) => {
    ...
    if (!isValidIndianPhone(phone)) {
        throw new https_1.HttpsError('invalid-argument', 'A valid 10-digit phone number is required.');
    }
    ...
    if (caller.role === 'admin' && roleWeight(requestedRole) >= roleWeight('admin')) {
        throw new https_1.HttpsError('permission-denied', 'Admins can only create reporter or public_user.');
    }
...
exports.trackShareInteraction = (0, https_1.onCall)(async (request) => {
    const uid = ensureAuth(request.auth?.uid);
    const input = toMap(request.data);
    const postId = String(input.postId ?? '').trim();
    if (postId.length === 0) {
...
exports.getTrendingHashtags = (0, https_1.onCall)(async (request) => {
...
exports.servePostPreview = (0, https_1.onRequest)(async (req, res) => {
    // Extract post ID from URL: /p/{postId}
    const pathParts = req.path.split('/').filter(Boolean);
    // Accept /p/{id} or just /{id}
    const postId = pathParts.length >= 2 && pathParts[0] === 'p'
        ? pathParts[1]
        : pathParts[0] || '';
...
<body>
  ...
  <div class="card">
    ...
    <div class="content">
      ...
      ${poemVerses ? `<div class="poem">${poemVerses.map((v) => escapeHtml(v)).join('<br>')}</div>` : ''}
      <div class="stats">
  ...
  <div class="footer">
    <p>&copy; ${new Date().getFullYear()} Focus Today. All rights reserved.</p>
        ...
        console.error('[servePostPreview] error:', error.message);
...
function buildDescription(title, author, category, likes) {
    const parts = [`By ${author}`];
    if (category)
        parts.push(`in ${category}`);
    if (likes > 0)
        parts.push(`${formatCount(likes)} likes`);
    return `${truncate(title, 80)} — ${parts.join(' • ')} — Focus Today`;
}
function buildMediaHtml(contentType, mediaUrl) {
    if (!mediaUrl) {
...
var notification_triggers_1 = require("./notification_triggers");
Object.defineProperty(exports, "onPostCreated", { enumerable: true, get: function () { return notification_triggers_1.onPostCreated; } });
Object.defineProperty(exports, "onPostStatusChange", { enumerable: true, get: function () { return notification_triggers_1.onPostStatusChange; } });
Object.defineProperty(exports, "onPostPublishedOutboxEnqueue", { enumerable: true, get: function () { return notification_triggers_1.onPostPublishedOutboxEnqueue; } });
Object.defineProperty(exports, "onPostResubmitted", { enumerable: true, get: function () { return notification_triggers_1.onPostResubmitted; } });
Object.defineProperty(exports, "onCommentCreated", { enumerable: true, get: function () { return notification_triggers_1.onCommentCreated; } });
Object.defineProperty(exports, "onUserRoleChanged", { enumerable: true, get: function () { return notification_triggers_1.onUserRoleChanged; } });
Object.defineProperty(exports, "onUserProfileIdentityChanged", { enumerable: true, get: function () { return notification_triggers_1.onUserProfileIdentityChanged; } });
Object.defineProperty(exports, "onReporterApplicationDecision", { enumerable: true, get: function () { return notification_triggers_1.onReporterApplicationDecision; } });
Object.defineProperty(exports, "onBreakingNewsCreated", { enumerable: true, get: function () { return notification_triggers_1.onBreakingNewsCreated; } });
Object.defineProperty(exports, "onMeetingCreated", { enumerable: true, get: function () { return notification_triggers_1.onMeetingCreated; } });
Object.defineProperty(exports, "onMeetingInterestCreated", { enumerable: true, get: function () { return notification_triggers_1.onMeetingInterestCreated; } });
Object.defineProperty(exports, "onMeetingInterestWritten", { enumerable: true, get: function () { return notification_triggers_1.onMeetingInterestWritten; } });
Object.defineProperty(exports, "onMeetingRsvpWritten", { enumerable: true, get: function () { return notification_triggers_1.onMeetingRsvpWritten; } });
Object.defineProperty(exports, "sendTestFcmToSelf", { enumerable: true, get: function () { return notification_triggers_1.sendTestFcmToSelf; } });
Object.defineProperty(exports, "sendMessageCampaign", { enumerable: true, get: function () { return notification_triggers_1.sendMessageCampaign; } });
Object.defineProperty(exports, "executeScheduledCampaigns", { enumerable: true, get: function () { return notification_triggers_1.executeScheduledCampaigns; } });
Object.defineProperty(exports, "sendMeetingReminders", { enumerable: true, get: function () { return notification_triggers_1.sendMeetingReminders; } });
Object.defineProperty(exports, "dispatchPublicPublishedPostDigest", { enumerable: true, get: function () { return notification_triggers_1.dispatchPublicPublishedPostDigest; } });
Object.defineProperty(exports, "cleanupStaleDevices", { enumerable: true, get: function () { return notification_triggers_1.cleanupStaleDevices; } });
