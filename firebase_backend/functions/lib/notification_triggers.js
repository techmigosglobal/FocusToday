...
exports.cleanupStaleDevices = exports.dispatchPublicPublishedPostDigest = exports.sendMeetingReminders = exports.executeScheduledCampaigns = exports.sendMessageCampaign = exports.sendTestFcmToSelf = exports.onMeetingCreated = exports.onMeetingRsvpWritten = exports.onMeetingInterestWritten = exports.onMeetingInterestCreated = exports.onBreakingNewsCreated = exports.onReporterApplicationDecision = exports.onUserProfileIdentityChanged = exports.onUserRoleChanged = exports.onCommentCreated = exports.onPostResubmitted = exports.onPostPublishedOutboxEnqueue = exports.onPostStatusChange = exports.onPostCreated = void 0;
...
function normalizeRoleTopicSuffix(value) {
...
function sanitizeTitle(raw, fallback) {
    ...
    if (title.length > CAMPAIGN_MAX_TITLE_LENGTH) {
        throw new https_1.HttpsError('invalid-argument', `Title must be <= ${CAMPAIGN_MAX_TITLE_LENGTH} characters`);
    }
    return title;
}
function sanitizeBody(raw, fallback = '') {
    ...
    if (body.length > CAMPAIGN_MAX_BODY_LENGTH) {
        throw new https_1.HttpsError('invalid-argument', `Body must be <= ${CAMPAIGN_MAX_BODY_LENGTH} characters`);
    }
    return body;
}
function normalizeCampaignType(raw) {
    const normalized = String(raw ?? 'all').trim().toLowerCase();
    if (!CAMPAIGN_ALLOWED_TYPES.has(normalized)) {
        throw new https_1.HttpsError('invalid-argument', "targeting.type must be one of: 'all', 'broadcast', 'role', 'topic'");
    }
    return normalized;
}
function normalizeCampaignRole(raw) {
    const normalized = normalizeRoleTopicSuffix(raw);
    if (!CAMPAIGN_ALLOWED_ROLES.has(normalized)) {
        throw new https_1.HttpsError('invalid-argument', "targeting.value for role must be one of: public_user, reporter, admin, super_admin");
    }
    return normalized;
}
function normalizeCampaignTopic(raw) {
    const topic = String(raw ?? 'new_content').trim() || 'new_content';
    if (!CAMPAIGN_ALLOWED_TOPICS.has(topic)) {
        throw new https_1.HttpsError('invalid-argument', `targeting.value for topic must be one of: ${Array.from(CAMPAIGN_ALLOWED_TOPICS).join(', ')}`);
    }
    return topic;
}
function buildCampaignResponse({ type, value, deliveryMethod, topic, targetUserCount, targetedCount, successCount, failureCount, invalidTokenCount, messageId, }) {
    return {
        ok: true,
        success: true,
...
function canonicalAuthorName(role, displayName) {
...
async function syncTopLevelAuthorFields({ collection, userId, authorName, authorAvatar, }) {
    let lastDoc = null;
    while (true) {
        let query = db
...
async function syncCollectionGroupAuthorFields({ collectionId, userId, authorName, authorAvatar, }) {
    let lastDoc = null;
    while (true) {
        let query = db
...
function actionDataJson(data) {
    if (!data || Object.keys(data).length === 0)
        return null;
    return JSON.stringify(data);
}
const PUBLIC_PUBLISHED_OUTBOX_PATH = 'notification_outbox/public_published_posts/items';
function publicPublishedOutboxCollection() {
    return db.collection(PUBLIC_PUBLISHED_OUTBOX_PATH);
}
function buildFiveMinuteWindowKey(date) {
...
async function createInAppNotificationsForUsers({ userIds, title, body, type, data, }) {
    if (userIds.length === 0)
        return;
...
async function getUserIdsByRoles(rawRoles) {
    const normalizedRoles = Array.from(new Set(rawRoles
        .map((role) => normalizeRole(role))
        .filter((role) => role.length > 0)));
    if (normalizedRoles.length === 0)
        return [];
    const queryRoles = Array.from(new Set(normalizedRoles.flatMap((role) => {
        switch (role) {
            case 'public_user':
...
function logDispatch(event, payload) {
...
async function sendFCMToTokens({ tokens, title, body, data, }) {
    ...
    for (const tokenChunk of chunkArray(uniqueTokens, 500)) {
        ...
        console.log(`[sendFCMToTokens] Batch sent: ${response.successCount}/${response.responses.length}`);
    ...
    if (invalidTokens.length > 0) {
        console.log(`[sendFCMToTokens] Found ${invalidTokens.length} invalid tokens to cleanup`);
    }
    console.log(`[sendFCMToTokens] Total sent: ${success}/${uniqueTokens.length}, failed: ${failed}`);
    return {
...
exports.onPostStatusChange = (0, firestore_1.onDocumentUpdated)('posts/{postId}', async (event) => {
...
exports.onPostPublishedOutboxEnqueue = (0, firestore_1.onDocumentWritten)('posts/{postId}', async (event) => {
...
exports.onPostResubmitted = (0, firestore_1.onDocumentWritten)('posts/{postId}', async (event) => {
    const before = event.data.before?.data();
    const after = event.data.after?.data();
    if (!before || !after)
        return;
    ...
    const afterStatus = after.status;
    const postId = event.params.postId;
    if (beforeStatus !== 'rejected' || afterStatus !== 'pending')
        return;
    const adminIds = await getAdminUserIds();
    if (adminIds.length === 0)
        return;
...
function parseMeetingStartUtcMs(meetingDate, meetingTime) {
...
async function getMeetingReminderAudienceUserIds(meetingRef) {
...
async function syncMeetingEngagementCounts(meetingId) {
...
exports.onUserRoleChanged = (0, firestore_1.onDocumentWritten)('users/{userId}', async (event) => {
    const before = event.data.before?.data();
    const after = event.data.after?.data();
    if (!before || !after)
        return;
...
exports.onUserProfileIdentityChanged = (0, firestore_1.onDocumentUpdated)('users/{userId}', async (event) => {
    const before = event.data.before?.data() ?? {};
    const after = event.data.after?.data() ?? {};
...
exports.onReporterApplicationDecision = (0, firestore_1.onDocumentUpdated)('reporter_applications/{applicationId}', async (event) => {
    const before = event.data.before?.data() ?? {};
    const after = event.data.after?.data() ?? {};
    ...
    const type = afterStatus === 'approved'
...
exports.onBreakingNewsCreated = (0, firestore_1.onDocumentCreated)('breaking_news/{newsId}', async (event) => {
    const news = event.data.data() ?? {};
    const newsId = event.params.newsId;
    if (news.is_active === false)
        return;
    const title = String(news.title ?? 'Breaking News');
    const body = String(news.subtitle ?? 'Tap to read the latest breaking update.');
    ...
    const audienceUserIds = Array.isArray(audience.user_ids)
        ? audience.user_ids.map((id) => String(id)).filter(Boolean)
        : [];
    const delayMinutes = Math.max(0, Number(news.notify_delay_minutes ?? 0) || 0);
    const data = {
        type: 'breaking_news',
        news_id: newsId,
    };
    if (delayMinutes > 0) {
        const scheduledAt = admin.firestore.Timestamp.fromDate(new Date(Date.now() + delayMinutes * 60 * 1000));
        const scheduledTargeting = audienceType === 'users'
...
exports.onMeetingInterestCreated = (0, firestore_1.onDocumentCreated)('meetings/{meetingId}/interests/{userId}', async (event) => {
    ...
    const userName = String(userData.display_name ?? 'A user').trim();
...
exports.onMeetingInterestWritten = (0, firestore_1.onDocumentWritten)('meetings/{meetingId}/interests/{userId}', async (event) => {
    const meetingId = String(event.params.meetingId ?? '').trim();
    if (!meetingId)
        return;
    await syncMeetingEngagementCounts(meetingId);
});
exports.onMeetingRsvpWritten = (0, firestore_1.onDocumentWritten)('meetings/{meetingId}/rsvps/{userId}', async (event) => {
    const meetingId = String(event.params.meetingId ?? '').trim();
    if (!meetingId)
        return;
    await syncMeetingEngagementCounts(meetingId);
...
exports.onMeetingCreated = (0, firestore_1.onDocumentCreated)('meetings/{meetingId}', async (event) => {
    ...
    const body = String(meeting.description ?? meeting.agenda ?? 'A new meeting is available. Check details.');
    const data = {
...
exports.sendTestFcmToSelf = (0, https_1.onCall)(async (request) => {
    try {
        ...
        if (!['admin', 'super_admin'].includes(userRole)) {
            throw new https_1.HttpsError('permission-denied', 'Only admin or super admin can run test push verification');
        }
        const payload = toRequestMap(request.data);
        const title = sanitizeTitle(payload.title, 'Focus Today Test Push');
        const body = sanitizeBody(payload.body, 'This is a manual FCM delivery verification message.');
        const data = {
            type: 'fcm_test',
        ...
        throw new https_1.HttpsError('internal', 'Failed to process test push');
    }
});
exports.sendMessageCampaign = (0, https_1.onCall)(async (request) => {
    try {
        const uid = request.auth?.uid;
        if (!uid) {
            throw new https_1.HttpsError('unauthenticated', 'Authentication required');
        ...
        const rawTargeting = toRequestMap(requestData.targeting ??
            {
        ...
        throw new https_1.HttpsError('internal', 'Failed to send campaign');
    }
});
exports.executeScheduledCampaigns = (0, scheduler_1.onSchedule)('every 1 minutes', async () => {
    ...
    for (const campaignDoc of campaignsSnap.docs) {
        ...
        try {
            ...
            const campaignData = campaign.data && typeof campaign.data === 'object'
                ? Object.fromEntries(Object.entries(campaign.data).map(([key, value]) => [key, String(value ?? '')]))
                : { type: 'campaign' };
            switch (targeting.type) {
                ...
                case 'users': {
                    const userIds = Array.isArray(targeting.value)
                        ? targeting.value.map((id) => String(id)).filter(Boolean)
                        : [];
                    const tokens = await getTokensForUserIds(userIds);
                    const delivery = await sendFCMToTokens({
                        tokens,
...
exports.sendMeetingReminders = (0, scheduler_1.onSchedule)('every 5 minutes', async () => {
    const now = Date.now();
    ...
    for (const meetingDoc of meetingsSnap.docs) {
        ...
        const startMs = parseMeetingStartUtcMs(meeting.meeting_date, meeting.meeting_time);
        if (startMs == null)
            continue;
...
exports.dispatchPublicPublishedPostDigest = (0, scheduler_1.onSchedule)('every 5 minutes', async () => {
    const now = admin.firestore.Timestamp.now();
...
exports.cleanupStaleDevices = (0, scheduler_1.onSchedule)('every 24 hours', async () => {
    const now = new Date();
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        try {
            ...
            console.error(`[cleanupStaleDevices] Error cleaning devices for user ${userId}:`, error.message);
        }
    }
    console.log(`[cleanupStaleDevices] Cleanup complete. Total devices removed: ${totalCleaned}`);
});
